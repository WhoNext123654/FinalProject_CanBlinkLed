                  ...
                  const can_user_config_t *config)
{
    ...
    status_t status = STATUS_ERROR;// thành công sẽ thành Status_Success
    uint8_t index = 0U; // chỉ số trạng thái can được cấp phát

    /* Define CAN PAL over FLEXCAN */
    #if (defined (CAN_OVER_FLEXCAN))
    if (instance->instType == CAN_INST_TYPE_FLEXCAN)// ktra có phải flex can ko
    {
        DEV_ASSERT(instance->instIdx < CAN_INSTANCE_COUNT); // hàm kiểm tra lỗi

        flexcan_user_config_t flexcanConfig; // cấu hình cho Flex can

        /* Allocate one of the FLEXCAN state structure for this instance */
        index = CAN_AllocateState(s_flexcanStateIsAllocated, // Mảng theo doi trạng thai da cap phat
                                  s_flexcanStateInstanceMapping,// anh xa chi so instance toi trang thai
                                  instance->instIdx);
...
#endif
//Cấu hình bitrate cho pha arbitration (dùng cho CAN 2.0 và CAN-FD), thiết lập thời gian bitrate
...
#if FEATURE_CAN_HAS_FD // Thiết lập bitrate nhanh hơn cho pha dữ liệu CAN-FD
...
#if FEATURE_CAN_HAS_PE_CLKSRC_SELECT // Chọn clock cho FlexCAN
...
#if FEATURE_CAN_HAS_DMA_ENABLE // Cấu hình phương thức truyền FIFO.
 ...
 *END**************************************************************************/
status_t CAN_Deinit(const can_instance_t * const instance) // hủy khởi tạo, giải phóng tài nguyên
{
    DEV_ASSERT(instance != NULL);
